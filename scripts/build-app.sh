...
ENTITLEMENTS="$(dirname "$0")/../Resources/AutoBlackout.entitlements"
...
if [ -n "${APPLE_DEVELOPER_ID_APPLICATION:-}" ]; then
  ...
  echo "  Using Developer ID with hardened runtime"
  codesign --force --deep --sign "$CODESIGN_IDENTITY" --entitlements "$ENTITLEMENTS" --options runtime "$APP_BUNDLE"
else
  # Fallback to ad-hoc signing for CI environments without keychain
  echo "  Using ad-hoc signing with hardened runtime (CI environment)"
